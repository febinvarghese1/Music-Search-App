...
function App() {
  ...
  const fetchApi = async () => {
    ...
    setTracks({songimg:data.tracks.hits[0].track.images.coverart,title:data.tracks.hits[0].track.title});
    console.log(data);
  ...
  return (
    <div className="Main-container">
      ...
      <input id="searchBar" type="text" placeholder="Search..." value={inputText} onChange={changeText} />
      <input id="submit" type="submit" onClick={submitHandler} />
      </div>
      <Artist name={artists.name} img={artists.avatar} songimg={tracks.songimg} title={tracks.title} />
    </div>
  );
}
